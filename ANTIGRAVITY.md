...
| `first-principles/` | 第一性原理思維 | 本 | 決策與思維 | blueprint |
...
- **2026-07-07 合併重構**：原本 4 個工具（energy-first/mind-offload/choice-loop/premortem-thinking）是各自獨立 repo＋各自 Pages 網址，已 vendored 進本 monorepo 成 `<slug>/index.html`＋`README.md`，入口卡片與工具頁尾「同系列」連結全改**相對路徑**（`./slug/`、`../slug/`）。原 Chinese 命名資料夾（能量管理.../認知卸載法.../探索選擇.../情境預演...）仍各帶 `.git` 留在磁碟當 legacy 原始碼、被 `.gitignore` 排除（連同 `.claude/`）。要改那 4 個工具請改 monorepo 內的 `<slug>/`，別再改 Chinese 資料夾。**2026-07-12 已下架 4 個舊 standalone Pages**（`iamkeith001.github.io/{choice-loop,energy-first,mind-offload,premortem-thinking}/` 現回 404），GitHub repo 本身保留、隨時可重開 Pages；monorepo 內已無任何連結指向它們。
- **入口頁「情境導引（從這裡開始）」（2026-07-07 加，2026-07-16 擴充至 13 條）**：hero 下方面板，回答使用者「先用哪個／怎麼串接／下一步是什麼」。JS 內 `SITUATIONS` 陣列寫死 13 條情境路徑（原 6 條＋沒電／習慣養不成／決策疲勞／資訊過載／學用落差／瞎忙無累積／資源變少），每條 3 步印章路徑（`href` 相對 `./id/`、seal 用工具 accent）＋結尾一句具體行動。點情境展開路徑、點印章開工具並 `recordVisit()`（共用 `prod_portal_visits`）。**加情境＝同步改三處：`SITUATIONS`、`SIT_KW`、跑 `scripts/build-semantic-index.mjs`**。
- **情境自由描述比對（2026-07-16 加，三層）**：情境區輸入框讓使用者用一句話描述卡點。①字典層：`SIT_KW` 每情境 25–45 條口語同義片段（英文小寫，查詢先 `toLowerCase`），命中即點亮 chip 展開路徑；沒中退到工具卡模糊比對（`data-keywords`／名稱／簡介，中文 2-gram），推薦前 3。②失手記錄：完全沒中且使用者 blur／Enter 時寫入 `prod_portal_misses`（上限 200 筆去重）——**定期看這個 key 養字典**。③智慧比對（可選增強）：miss 時出現「試試智慧比對」鈕，點了從 jsDelivr 動態載入 transformers.js＋`Xenova/bge-small-zh-v1.5`（quantized 約 25MB，transformers.js 自帶快取，之後可離線），查詢向量對 `semantic-index.json`（13 情境＋43 工具向量，SW 會快取）算內積；門檻採邊際規則（`th.sit=0.55` 或 `≥0.50 且領先第二名 0.04`、工具 `0.50`，2026-07-16 以 10 句實測校準）。開關記在 `prod_portal_semantic_on`，開過之後 miss 自動跑。**新增情境或工具後要重跑 `scripts/build-semantic-index.mjs` 重生索引並 commit**（依賴 `@xenova/transformers@2.17.2`，版本要與前端 CDN 一致）。
...
- **PWA 離線（2026-07-12 加）**：根目錄 `manifest.webmanifest`＋`icon.svg`＋`sw.js`。SW precache 入口＋全部工具頁，之後 stale-while-revalidate（先回快取、背景更新，**改版後第二次進場才拿到新版**，屬預期行為）。**更新提示（2026-07-12 加）**：SW 背景更新導航頁面時比對新舊 etag，變了就 `postMessage({type:"sw-updated",url})` 給所有視窗；頁面端（入口＋40 工具的註冊區塊）比對 pathname 後顯示「此頁有新版本／重新整理」浮條（`#swUpdateBar`，工具版離底 84px 避開 tabbar、入口版 20px）。離線且該頁沒進快取時導航請求 fallback 回入口頁。入口與全部 40 工具檔尾都有註冊區塊（工具用 `../sw.js`）。**新工具要把 `"./slug/"` 加進 `PRECACHE`**（check.mjs 會擋）；sw.js 位元組一變瀏覽器就會重裝並重抓 precache，通常不必手動 bump `CACHE` 版本，要強制全部重抓時才改 `prod-sys-v1` 版號。
